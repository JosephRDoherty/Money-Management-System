...
function payPeriodCalc(startDate, endDate){
    // Finds all bills due within a given range
    let billArray = [];
...
function typeFinder(type="Credit Card", array=billList){
    // returns an array of a given type
...
function arrayCostCalc(billArray){
    // returns the total amount dude for the given array
    let amountDue = 0;
...
function toDate(year, month, day){
    // more easily create a Date object
    var result = new Date(year, month, day);
    return result;
}

function removeDay(date){
    // gets rid of the 'Mon' 'Tue' 'Fri' day thing
...
function daysTilNextPayday(){
    // countdown days til next payday
...
function fortnightBudgetTester(amountSaved = 1594, array = calcFuturePaydayCost(26)){
    // used to manually test stuff
    // deprecated, but might still be useful sometimes
...
function calcFuturePaydayCost(lookAhead, printToConsole = false, includeBills = false){
    ...
    let averageCost = 0;
    let averageProfit = 0;
    let payCalendar = []; // This is what gets returned
